...
// when saving a double to a file in text format, use the maximum possible precision & loading a hex from a file
...
inline my_ifstream& operator>>(my_ifstream& is, double& v) {
    ...
    is >> s;
    union { double d; uint64_t u; } tmp;
    std::istringstream(s) >> std::hex >> tmp.u;
    v = tmp.d;
    return is;
}
/***********************************
...
inline std::ostream& operator<<(std::ostream& os, const Date& d) //cout (to the cmd)
{
    os << d.to_string(true);
    return os;
}

inline my_ofstream& operator<<(my_ofstream& os, const Date& d) //to the file
{
...
inline my_ifstream& operator>>(my_ifstream& is, Date& v)
{
    ...
    is >> tmp;
    if (tmp.size() != 8) {
        unsigned s = std::atoi(tmp.c_str());// 200000
        v.init(s);
        return is;
    }
