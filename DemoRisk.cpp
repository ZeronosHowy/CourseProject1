...
void run(const string& portfolio_file, const string& risk_factors_file, const string& base, const string& fixing_file)
{
    // load the portfolio from file
    portfolio_t portfolio = load_portfolio(portfolio_file); //PortfolioUtils.cpp
    // save and reload portfolio to implicitly test round trip serialization
    save_portfolio("portfolio.tmp", portfolio); //PortfolioUtils.cpp
    portfolio.clear();
    portfolio = load_portfolio("portfolio.tmp");

    // display portfolio
    print_portfolio(portfolio); //PortfolioUtils.cpp & Trade.h & TradePayment.h

    // get pricers
    std::vector<ppricer_t> pricers(get_pricers(portfolio, base)); //PortfolioUtils.cpp
    ...
    {
        auto prices = compute_prices(pricers, mkt, fds); //PortfolioUtils.cpp
    ...
    {   // Compute PV01 (i.e. sensitivity with respect to interest rate dV/dr)
        std::vector<std::pair<string, portfolio_values_t>> pv01(compute_pv01(pricers, mkt, fds));  // PV01 per trade
...
int main(int argc, const char **argv)
{
    ...
    string portfolio, riskfactors, base, fixing;
    base = "USD";
    if (argc % 2 == 0)
        usage();
    ...
    try {
        run(portfolio, riskfactors, base, fixing);
